...
public class TestTemplate extends Test {
    ...
    public TestTemplate() {
        ...
        /**
         * 노드 3은 다른 next test가 있음 아직 추가 안함
         */
        Node node3 = Node.builder()
                .name("누운자세 교차 상지 패턴 검사")
                .imageFileName("2_3")
                .build()
                .addChild("O", Child.builder().description("패턴1을 위한 분리된 자세 &/ or 어깨대 SMCD. 척추신전 하위 검사로 간다.").build())
                .addChild("X", Child.builder().description("패턴1의 기능적 어깨 패턴 SMCD. 척추신전 하위 검사로 간다.").build())
                .addChild("P", Child.builder().description("통증 치료").build());
    ...
    public static Test getInstance() {
        if (TestTemplate.instance == null) { TestTemplate.instance = new TestTemplate();}
        return instance;
    }
}
