...
public class SLS1 extends mainPackage.v2.tests.Test {
    ...
    public SLS1() {
        Node node5 = Node.builder()
                ...
                .imageFileName("6_5")
                .build()
                .addChild(FN, Child.builder().color(BLUE).description("체중지지 척추 &/or 고관절/코어 SMCD - (만약 고관절 신전이 DN이면 이것을 먼저 치료한다).")
                        .nextTest(new Node[]{SLS2.getInstance().getHead()}).build())
                .addChild(DN, Child.builder().color(BLUE).description("체중지지고관절 &/or 코어 SMCD (만약 고관절 신전 &/or 어깨 굴곡이 DN이면 이것을 먼저 치료한다.)")
                        .nextTest(new Node[]{SLS2.getInstance().getHead()}).build())
        ...
        Node node4 = Node.builder()
                ...
                .addChild(DN, Child.builder().color(BLUE).description("펀더먼털 고관절 &/ or 코어 SMCD")
                        .nextTest(new Node[]{SLS2.getInstance().getHead()}).build())
                .addChild(P, Child.builder().color(BLUE).description("통증 치료")
                        .nextTest(new Node[]{SLS2.getInstance().getHead()}).build());
        ...
        Node node3 = Node.builder()
                ...
                .imageFileName("6_3")
                .build()
                .addChild(FN, Child.builder().color(BLUE).nextTest(new Node[]{SLS2.getInstance().getHead()}).build())
    ...
    public static Test getInstance() {
        if (SLS1.instance == null) { SLS1.instance = new SLS1();}
        return instance;
    }
}
