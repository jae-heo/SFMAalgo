...
public class MultiExtension2 extends Test {
    ...
    public MultiExtension2() {
        Node node6 = Node.builder()
                ...
                .imageFileName("4_11")
                .build()
                .addChild(FN, Child.builder().color(GREEN).description("코어 SMCD &/or 능동 고관절 신전 SMCD").build())
                .addChild(DN, Child.builder().color(GREEN).description("펀더먼털 신전패턴 SMCD").build())
        ...
        Node node5 = Node.builder()
                ...
                .imageFileName("4_11")
                .build()
                .addChild(FN, Child.builder().color(GREEN).description("고관절 신전 기능제한의 표시가 있으면 체중지지 고관절 &/or" +
                        " 척추 하부 흉부 신전 SMCD &/or 발목 배측굴곡 제한이 있다. 없다면 고관절 신전은 정상이다 (ODS & SLS을 검사" +
                        "한다)").node(node6).build())
                .addChild(DN, Child.builder().color(GREEN).description("펀더먼털 신전패턴 SMCD").build())
                .addChild(P, Child.builder().color(RED).description("통증치료").build());
        ...
        Node node4 = Node.builder()
                ...
                .addChild(DN, Child.builder().color(GREEN).description("고관절 신전 JMD &/or TED").build())
                .addChild(P, Child.builder().color(RED).description("통증치료").build());
        ...
        Node node2 = Node.builder()
                ...
                .addChild(TOMASFN1, Child.builder().color(GREEN).description("하부 전방 사슬 TED").build())
                .addChild(TOMASFN2, Child.builder().color(GREEN).description("하부 외측 사슬 TED").build())
                .addChild(TOMASFN3, Child.builder().color(GREEN).description("하부 전방 및 외측 사슬 TED").build())
                .addChild(DN, Child.builder().color(GREEN).description("고관절 신전 JMD &/or TED and/or 코어 SMCD").build())
                .addChild(P, Child.builder().color(RED).description("통증치료").build());
        ...
        Node node1 = Node.builder()
                ...
                .addChild(DN, Child.builder().color(ORANGE).node(node2).description("고관절/SI JMD &/or TED &/or 코어 SMCD" +
                        " * (고정된 FABER로 분별한다) 고관절과 SI의 국소부위 생역학검사를 실시한다.").build())
                .addChild(P, Child.builder().color(ORANGE).node(node2).description("고관절/SI JMD &/or TED &/or 코어 SMCD" +
                        " * (고정된 FABER로 분별한다) 고관절과 SI의 국소부위 생역학검사를 실시한다.").build());
